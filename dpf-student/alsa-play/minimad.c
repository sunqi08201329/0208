...
int minimad(int fd_mp3, FILE *fp_tmp)
{

	fp = fp_tmp;
	int file_len = lseek(fd_mp3, 0, SEEK_END);

	void *fdm;
	fdm = mmap(0, file_len, PROT_READ, MAP_SHARED, fd_mp3, 0);
	if (fdm == MAP_FAILED)
		return 3;

	decode(fdm, file_len);

	if (munmap(fdm, file_len) == -1)
		return 4;
